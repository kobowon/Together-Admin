...
router.put('/real-matching/location',function (request,response) {
    ...
    console.log('ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
...
router.get('/helpers/push',function (req,res) {
    ...
    var stmt = 'select token from device where id in (select deviceId as id from user where (userType=? AND SQRT(POW(latitude-?,2)+POW(longitude-?,2))<0.04))';
...
router.put('/record', recordUpload.single('recordfile'), function (req, res) {// userfile이 form data의 key 가 된다.
    ...
    var stmt = 'UPDATE volunteeritem SET helpeeScore = ?, helpeeFeedbackRecord = ?,helpeeFeedbackContent=? where volunteerId = ?';
    var params = [req.body.helpeeScore, recordPath,req.body.helpeeFeedbackContent, req.body.volunteerId];
    connectionPool.getConnection(function (err, connection) {
        // Use the connection
        connection.query(stmt, params, function (err, result) {
            // And done with the connection.
